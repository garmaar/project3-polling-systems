...
The validated simulator was then used to study how polling performance changes with traffic load, number of stations, and asymmetric traffic conditions. The results confirm that exhaustive polling consistently achieves lower average waiting times than limited polling, and that it handles asymmetric traffic more fairly, as shown by the extension scenario and the corresponding Jain's fairness index. The complete analysis and discussion are available in the final report.
